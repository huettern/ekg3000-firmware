...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
    ...
    // Message Type
    fixed_header[0] = MQTT_MSG_CONNECT & 0xf0;
	...
	uint16_t offset = 0;
	int packetsize = sizeof(fixed_header)+sizeof(var_header)+payload_len;
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain, uint8_t qos, uint16_t* message_id) {
   ...
   if (remainLen <= 127) {
	...
	int packetsize = sizeof(fixed_header)+sizeof(var_header)+msglen;
	// uint8_t packet[sizeof(fixed_header)+sizeof(var_header)+msglen];
	// memset(packet, 0, packetsize);
	memset(packet, 0xBB, MAX_PACKET_SIZE);
	memcpy(packet, fixed_header, sizeof(fixed_header));
...
int mqtt_pubrel(mqtt_broker_handle_t* broker, uint16_t message_id) {
	...
	packet[0] = MQTT_MSG_PUBREL | MQTT_QOS1_FLAG; // Message Type, DUP flag, QoS level, Retain
	packet[1] = 0x02; // Remaining length
