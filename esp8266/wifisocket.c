/**
 * @file       wifisocket.c
 * @brief      UNIX style socket commands using the esp module
 * @details    Source: https://github.com/vpcola/MikroChibiOS
 * @author     Noah Huetter (noahhuetter@gmail.com)
 * @date       1 December 2016
 * 
 *
 * @addtogroup WIFISOCKET
 * @brief UNIX style socket
 * @{
 */
...
char * inet_ntoa_r(const in_addr *addr, char *buf, int buflen)
{
    ...
    uint32_t buff = s_addr;
    // dirty fix: inverse order of s_addr
    // s_addr = ((buff & 0x11000000) >> 24) | ((buff & 0x00110000) >> 8) | ((buff & 0x00001100) << 8) | ((buff & 0x00000011) << 24);
...
int bind(int sockfd, const sockaddr * addr, long addrlen)
{
  ...
  sockaddr_in * source;
  esp_channel_t * channel = getChannel(sockfd);
...
{
  (void)flags;
  return channelSend(sockfd, msg, len);
}
