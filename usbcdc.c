...
static void cmd_threads(BaseSequentialStream *chp, int argc, char *argv[]) {
  ...
  chprintf(chp, "    addr    stack prio refs     state    time      wa_size     free name\r\n");
  tp = chRegFirstThread();
  do {
    chprintf(chp, "%08lx %08lx %4lu %4lu %9s %8lu %11lu %8lu %s\r\n",
...
/**
 * @brief      Performs an SD card benchmark
 *
 * @param      chp   pointer to the output stream
 * @param[in]  argc  argument count
 * @param      argv  argument list
 */
static void cmd_sdcbenchmark(BaseSequentialStream *chp, int argc, char *argv[]) {
  (void)chp;
  (void)argc;
  (void)argv;
  sdcBenchmark(chp, atoi(argv[0]));
}

/**
 * @brief      starts the WPS routine
 *
 * @param      chp   pointer to the output stream
 * @param[in]  argc  argument count
 * @param      argv  argument list
 */
static void cmd_wps(BaseSequentialStream *chp, int argc, char *argv[]) {
  (void)chp;
  (void)argc;
  (void)argv;
  wifiStartWPS();
...
#if _USE_MKFS
static void cmd_sdformat(BaseSequentialStream *chp, int argc, char *argv[]) {
  (void)chp;
  (void)argc;
  (void)argv;
  (void)sdcMkfs();
...
/**
 * @brief      sample a given number of samples
 *
 * @param      chp   pointer to the output stream
 * @param[in]  argc  argument count
 * @param      argv  argument list
 */
static void cmd_anSample(BaseSequentialStream *chp, int argc, char *argv[]) {
  (void)chp;
  (void)argc;
  (void)argv;
  if(argc)
  {
    anSampleN(atoi(argv[0]));
  }
}
...
static const ShellCommand commands[] = {
		{"mem", cmd_mem},
		{"threads", cmd_threads},
    {"ansample", cmd_anSample},
    {"sdcbenchmark", cmd_sdcbenchmark},
    {"wps", cmd_wps},
#if _USE_MKFS
    {"sdformat", cmd_sdformat},
#endif // #if _USE_MKFS 
		{NULL, NULL}
};
